class Vehiculos {
  ...
  buscarPorPrecio(orden) {
    ...
    if (typeof orden === "string") {
      ...
      if (orden === "caro") {
        ...
        throw new Error(
          "No se ha recibido una orden válida en método 'buscarPorPrecio'."
        );

      // Mostrando resultado por consola.
      console.log(
        `Vehículo más ${orden}: ${this.capitalizar(
          vehiculo?.marca
        )} ${this.capitalizar(vehiculo?.modelo)}`
      );
    } else
      throw new TypeError(
        "No se ha recibido un string en método 'buscarPorPrecio'."
      );
  }
  ...
  buscarLetraEnModelo(letra) {
    ...
    if (typeof letra === "string") {
      ...
      if (letra.length != 1)
        throw new Error(
          "Se ha recibido más de una letra como argumento en método 'buscarLetraEnModelo'."
        );

      for (let v of this.vehiculos) {
        let modelo = String(v.modelo).toLowerCase(); // El modelo puede ser un n° o estar en mayúsculas...
        if (modelo.includes(letra))
          // Buscando letra en modelo.
          vehiculo = v;
      }

      // Mostrando resultado por consola.
      console.log(
        `Vehículo que contiene en el modelo la letra ‘${letra.toUpperCase()}‘: ${this.capitalizar(
          vehiculo?.marca
        )} ${this.capitalizar(vehiculo?.modelo)} \$${this.formatear(
          vehiculo?.precio
        )}`
      );
    } else
      throw new TypeError(
        "No se ha recibido como argumento un string en método 'buscarLetraEnModelo'."
      );
  }
  ...
  mostrar(campos) {
    ...
    if (campos === undefined) {
      // Si no se recibe campos en particular como argumento, obtener todos.
      for (let v of this.vehiculos) {
        for (let p in v)
          if (p !== "precio")
            // Mientras no sea 'precio', mostrar doble barra al final.
            datos += `${this.capitalizar(p)}: ${this.capitalizar(v[p])} // `;
          // Si es 'precio', agregar '$' antes del precio y hacer salto de linea.
          else datos += `${this.capitalizar(p)}: \$${this.formatear(v[p])}\n`;
      }
    }
    // En caso de recibirse un array, agregar del vehículo solo los campos pasados de ese array.
      ...
      throw new Error(
        "Variable 'campos' no es del tipo array/objeto en método 'mostrar'."
      );
  ...
  capitalizar(cadena) {
    ...
    if (typeof cadena === "string") {
      ...
      throw new TypeError(
        "No se ha recibido un string o número a parsear en método 'capitalizar'."
      );
  }
  ...
  formatear(precio) {
    ...
    if (typeof precio === "number") {
      ...
      throw new TypeError(
        "No se ha recibido un número (entero o float) en método 'formatear'."
      );
  }
  ...
  ordenarPorPrecio(orden = "mayor") {
    ...
    if (typeof orden === "string") {
      ...
      if (orden === "mayor") this.vehiculos.sort((a, b) => b.precio - a.precio);
      else if (orden === "menor")
        this.vehiculos.sort((a, b) => a.precio - b.precio);
      else
        throw new Error(
          "No se ha recibido una orden válida en método 'ordenarPorPrecio'."
        );
    } else
      throw new TypeError(
        "No se ha recibido un string en método 'ordenarPorPrecio'."
      );
  }
}
